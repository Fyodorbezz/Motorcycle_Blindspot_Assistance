...
class Motor{
  ...
  void set_actual_speed(){
    if (actual_speed == 0){
      ...
      analogWrite(A_PIN, actual_speed);
      digitalWrite(B_PIN, 0);
    }
  }
...
Motor motor_1(MOTOR1_A, MOTOR1_B, 1.52, -643, 0.3, 0.2, 0.05);
Motor motor_2(MOTOR2_A, MOTOR2_B, 1.59, -743, 0.3, 0.2, 0.05);
...
void loop() {
  ...
  if(digitalRead(BRAKE1) || digitalRead(BRAKE2)){
    motor_1.real_speed = 0;
    motor_2.real_speed = 0;
  }
